...
$('#jstree').on("changed.jstree", function(e, data) {
	if (data.selected == -1) {
		...
		$("#exampleTable").bootstrapTable('selectPage',1); //重新查询，跳转到第一页，比如page=5会变成page=1
	} else {
		var opt = {
			query : {
				roleId : data.selected[0],
				offset : 0,
			}
		}
		$("#roleId").val(data.selected[0]);
		console.log("sele roleId--->",data.selected[0],$("#roleId").val())
		$('#exampleTable').bootstrapTable('refresh',opt);
		$("#exampleTable").bootstrapTable('selectPage',1); //重新查询，跳转到第一页，比如page=5会变成page=1
	}
